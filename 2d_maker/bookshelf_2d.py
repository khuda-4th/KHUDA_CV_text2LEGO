...
modelnet40_folder = 'ModelNet40/bookshelf'
...
for split in ['train', 'test']:
    ...
    for file_name in os.listdir(data_folder):
        if file_name.endswith('.off'):
            ...
            z_model_center = (model_extents[0] + model_extents[1]) / 2.0 + max_bound[2] / 2.0
            pose = np.array([
                [1.0, 0.0, 0.0, density_center[0]],  # X 좌표 조정
                [0.0, 1.0, 0.0, density_center[1] - (model_extents[0] + model_extents[1] + model_extents[2]) / 2.7],  # Y 좌표 조정
                [0.0, 0.0, 1.0, z_model_center],  # Z 좌표 조정
                [0.0, 0.0, 0.0, 1.0]
            ])

            # Yaw, pitch, roll angles in radians
            yaw, pitch, roll = np.radians(0), np.radians(0), np.radians(65)
